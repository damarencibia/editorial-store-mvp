<template>
  <form @submit.prevent class="rounded-lg border border-border bg-surface-2 p-6 space-y-6">
    <h2 class="font-heading text-lg font-semibold">Editar perfil</h2>

    <div class="space-y-1.5">
      <label for="fullName" class="text-sm text-text-muted">Nombre</label>
      ...
      <p v-if="nameSuccess" class="text-xs text-green-400 mt-1">{{ nameSuccess }}</p>
      <p v-if="nameError" class="text-xs text-red-400 mt-1">{{ nameError }}</p>
    </div>

    <div class="space-y-1.5">
      <p class="text-sm text-text-muted">Email</p>
      <p class="text-sm text-text-primary">{{ userEmail }}</p>
      <p class="text-xs text-text-dim">El email no se puede cambiar desde aquí.</p>
    </div>

    <hr class="border-border" />

    <h3 class="font-heading text-base font-semibold">Cambiar contraseña</h3>
    ...
    <div class="space-y-1.5">
      <label for="confirmPassword" class="text-sm text-text-muted">Confirmar nueva contraseña</label>
      <div class="relative">
        <input
          id="confirmPassword"
